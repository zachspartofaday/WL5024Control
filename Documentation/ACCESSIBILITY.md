...
The UI test target covers all six visible destinations at regular and minimum window sizes, Ready, read-only, and Experimental states, single-identity checkbox and pop-up-button roles, explicit "Set value for …" experimental menus, the menu-bar read-only explanation, selectable diagnostic values, the discovery action, completion summary, and cancellation partial-result text, visible export progress, save-panel cancellation, result dismissal, and real first-responder return to the export button. Appearance profiles (light, dark, increased contrast, reduced transparency) are launch-smoke coverage only: each launches, exposes one control, and retains a screenshot. Default, minimum, and launch-smoke screenshots are retained in the test result bundle.
