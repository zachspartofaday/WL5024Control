# Development and hardware diagnostics

For installation and supported features, see the [README](../README.md). Run the commands below from the repository root unless a command changes directories.
...
Files matching `*.wl5024log.json` and the local `Artifacts` directory are ignored by Git and must not be committed. Captures contain device identifiers and raw protocol traffic: review and redact them before sharing, and use an agreed private channel for protocol analysis rather than a public issue attachment.
...
See [PROTOCOL.md](PROTOCOL.md) for the recovered wire format and validation checklist.
...
Manual accessibility scenarios and their expected outcomes are in [ACCESSIBILITY.md](ACCESSIBILITY.md). Physical command qualification remains governed by [PROTOCOL.md](PROTOCOL.md).

## Release maintenance

See [RELEASING.md](RELEASING.md) for packaging, notarization, and public repository protections.
